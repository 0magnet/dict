...
# committed under docs/, and it takes about two seconds.
#
...
GOFILES := $(shell find . -name '*.go' -not -path './docs/*')

.PHONY: demo demo-tinygo serve clean

demo: docs/dict.wasm docs/wasm_exec.js ## build the demo page (standard Go, ~2s)

docs/dict.wasm: $(GOFILES)
...
demo-tinygo: ## build the demo with TinyGo instead (a third the size, minutes not seconds)
	...
	GOTOOLCHAIN=$$(sh scripts/tinygo-toolchain.sh) tinygo build -target wasm -tags dictlite \
		-no-debug -opt=z -o docs/dict.wasm ./web/term/
